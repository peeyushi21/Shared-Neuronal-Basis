...
%nested data: neurons * tasktype (1 SSRT 2 choice) * trials (1 go 2 stop) *
%time
...
for neuroni = 1:length(neuroncumT)
    ...
    %1= stop signal task, 2= choice tas
    % k
    ...
    % ind3 = find(data(:,20) == min(nbins1_ne_col)); %low
    % ind4 = find(data(:,5) <= median(data(:,5)));
    % ind12 = intersect(intersect(intersect(ind1,ind2),ind3),ind4);
    %ind12 = rangeT(intersect(ind1,ind2)).';
    %ind123 = rangeT(intersect(ind12,ind3));
    raw_data=ind4;
    ...
    ind2 = ind1(ind1(:,15004)==0,:);
    ind3 = ind2(ind2(:,15007)==0.2000,:); %1= stop cue present, 0= absent (based on stop stim presentation)
    ind4 = ind2(ind2(:,15007)==0.0100,:);
    % ind3 = find(data(:,20) == min(nbins1_ne_col)); %low
    % ind4 = find(data(:,5) <= median(data(:,5)));
    % ind12 = intersect(intersect(intersect(ind1,ind2),ind3),ind4);
    %ind12 = rangeT(intersect(ind1,ind2)).';
    %ind123 = rangeT(intersect(ind12,ind3));
    ind = vertcat(ind3,ind4);
    indd = ind(ind(:,15009) >= 0.5000,:);
...
for neuroni = 1:length(neuroncumT)
    ...
    ind3 = ind2(ind2(:,15008)==0.1000,:);%1= stop cue present, 0= absent (based on stop stim presentation)
    ind4 = ind3(ind3(:,15009)>=0.5000,:);
    % ind3 = find(data(:,20) == min(nbins1_ne_col)); %low
    % ind4 = find(data(:,5) <= median(data(:,5)));
    % ind12 = intersect(intersect(intersect(ind1,ind2),ind3),ind4);
    ...
    ind1 = data(data(minT:maxT,15003)==2,:); %1= stop signal task, 2= choice task
    ind2 = ind1(ind1(:,15004)==1,:);
    ind3 = ind2(ind2(:,15008)==0.2000,:);
    ind4 = ind2(ind2(:,15008)==0.0100,:);
    ind = vertcat(ind3,ind4);
    indd = ind(ind(:,15009) >= 0.5000, :);
   %1= reject cue present, 0= absent (based on stop stim presentation)
    %ind3 = vertcat(indp,indq);
    %ind12 = rangeT(intersect(ind1,ind2));
    %ind123 = rangeT(intersect(ind12,ind3));
    ...
    for j = 1:trials2
        ssd = indd(j,15005);
        tstart_stop = 5000-ssd;
        raw_data(j,:) = indd(j,mint_stop+tstart_stop:maxt_stop+tstart_stop);
        raw_data(any(isnan(raw_data), 2), :) = [];
        raw_data( all(~raw_data,2), : ) = [];
    end
...
margNames = {'Accept/Reject', 'Med/Low_or_high', 'Condition-independent', 'T/A Interaction'};
...
timeEvents = [0.5000,0.6500,0.7700];
